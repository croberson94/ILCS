...
% Intro Section
The design requirements for the ILCS system that were tested include the automatic light compensation algorithm, single communication bus interface to address each light fixture, and the graphical user interface. \\

% Verification Section
%% Automatic Light Compensation Algorithm

%% Single Communication Bus Interface
The single communication bus interface was tested by connecting the 4Duino graphical user interface, Arduino UNO, and Infineon RGB Driver boards to the SCL and SDA data lines. Communication involved the sending of a single byte between the 4Duino and Arduino devices. The byte can be separated into three categories: sector (bits 0-1), offset (bits 2-5), and offcode (bits 6-7). The Arduino serial monitor provides a visualization of the byte that is sent between the 4Duino and the Arduino UNO, and in testing accounted for correct signals being sent by witnessing the data value through the seria monitor. \\

%% Graphical User Interface
The current graphical user interface provided by the 4Duino was tested by iterating through several design interface choices based on group feedback. Iterations of graphic design (see appendix A) and the current design (see appendix B) are available for reference. The current design displays to the user the system manufacturer name, user network connection, confirmation of user network connection, GMT timezone and lighting quadrant offset times as plain text. This plain text is updated every 5 seconds as the GMT time is retrieved from the NTP server, confirmed by reading the displayed GMT Time. There are 15 iWinbuttons that act as toggles and are controlled by the user. The 8 offset buttons act as positive (+1) and negative (\-1) for each of the four lighting quadrants. Each quadrant has its own on/off toggle button located in the center of the screen. The profile button works in conjunction with the save button, loading the preserved offsets of the save button. There are a possible of two separate profiles for all four quadrants. The reset button does what it indicates, resetting the byte that is sent and iWinbutton conditions. Placement of the toggle buttons were made to be within 20 pixels for ease of use and placed accordingly on the 240x320 resolution of the 4Duino display. \\

%% Alarm System
%% Lighting & Temperature Sensors
The lighting and temperature sensors that regulate the lumen levels and celsius of each lighting quadrant were tested by plugging in the pins to the GPIO pins of the Arduino UNO and reading the values. The testing environment for the sensors was done in a temperate classroom environment. Lighting values to detect the lumens were gathered to be ... Temperatures are noted to be... within this environment, for more results concerning the testing of temperature sensors (see appendix A). \\

%% Power Supply Output/Wiring
% Strengths and Weaknesses Section
The main strengths of our design are the ...
Areas of improvement for future iterations on this design are... \\
...
Areas of improvement for future iterations on this design are microcontroller selection with greater range of dynamic memory (including external expansion capabilities), physical space for analog push buttons, DNP3 for device communications, and all-in-one PCB desgin for the wiring. A greater dynamic range of memory is necessary for the program that is written, as the amount of global variables needed to carry out the program were seen to reach maximum capacity given the microncontroller we used. The 4Duino graphical user interface used 15 touchscreen buttons due to convenience of materials availabile. Communications between the 4Duino, Arduino and Infineon RGB drivers utilize I2C, however, an alternative method of communication would be to use a distributed network protocol such as DNP3 or Modbus. I2C was readily available and used by the devices we have within our network. An all-in-one PCB design would contain traces for the wiring inputs and components that act as our microcontroller. \\
