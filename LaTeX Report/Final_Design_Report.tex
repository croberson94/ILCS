...
The design requirements for the ILCS system that were tested and confirmed as solutions include the automatic light compensation algorithm, centrally controlled sensor feedback, alarm system, voltage regulation, single communication bus for fixture addressing, visual status display, and touchscreen user interface for customized sleep cycles. \\

% Verification Steps Section (Requirement, Procedure/Protocol, Results)


%% Centrally Controlled Sensor Feedback
...
%% Alarm System
The alarm system determines any incorrect sensor input in for our system. It will trigger when there is light degradation, or temperature and alert the user for correction. The design was made using the Adafruit FX mini sound board along with an operational amplifier using the LM386 class AB audio amplifier IC to build a simple mono 1W audio amplifier. Both are individually powered by the 5V power supply and the sound board allowed for a simple and dependable way to add audio effects to our project. The audio uses compressed Ogg files or uncompressed WAV files that is easy to install. The sound board has many features, for instances five different trigger effects like looping or latching but all we used it for was basic trigger function. Basic triggering happens as the matching trigger pin is connected to ground, and in return the stored audio file plays. \\

The design was simple but getting the results we wanted was not. We began by copying audio files over to the Sound Board then powered the board by connecting the 5V to Vin and ground to ground. The LM386 amplifier used two 100uF capacitors and one 1000uF capacitor with two 1K Ohm resistors. The issues we ran into was in the output of the amplifier. With a multimeter we determined that 5V were being supplying but the output was lower than that and no sounds was being produced. We concluded that the 5V was too little to supply the amp and it needed more voltage. Rather than having different voltage sources, we decided to select more simplistic alternative with the Texas Instrument TPA2005D1 mono class-D audio amplifier. It can drive an 8 Ohm speaker at up to 1.4W. This device contains a PWM signal clocked at 250kHz which is much higher than the frequencies being encoded at less than 20kHz. Finally, after applying the 5V from the source to the sound board and the amplifier, we connected the audio output R and GND pins of the sound board to the IN positive and negative terminals respectively of the TPA2005D1 amp, and the OUT terminals are connected to the terminals of the 4-Ohm speaker we are using. The results from this device were successful and efficient for the alarm system, we were able to trigger the pin and set the alarm to output the sound we wanted at the specific time of an analog input. \\
...
%% Single Communication Bus for Fixture Addressing
...
%% Touchscreen User Interface for Customized Sleep Cycles
...
%% Automatic Light Compensation Algorithm
...
% Strengths and Weaknesses Section (Overall Thoughts, Improvements)
...
// DECLARATIONS
#include <Wire.h>
#define SSID "HCCPublic" // replace with your user network
#define PASSWORD "" // replace with your password (optional)
...
int iWinbutton13, iWinbutton14, iWinbutton15;
byte quad_offset = B00000000;
byte quad_offset_2 = B00000000;
byte quad_offset_3 = B00000000;
byte quad_offset_4 = B00000000;

// SAVED PARAMETERS
int SGMT0, SGMT1, SGMT2, SGMT3, SGMT4;
int SGMO1, SGMO2, SGMO3, SGMO4, SGMO5;
byte squad_offset;
byte squad_offset_2;
byte squad_offset_3;
byte squad_offset_4;
...
//hstrings = Display.file_Open("NTP2~1.txf", 'r') ;                            // Open handle to access uSD strings, uncomment if required
  ...
  Display.img_ClearAttributes(hndl, iWinbutton13, I_TOUCH_DISABLE); // Winbutton13 set to enable touch, only need to do this once
  Display.img_Show(hndl, iWinbutton13);  // Winbutton13 show button, only do this once
  Display.img_ClearAttributes(hndl, iWinbutton14, I_TOUCH_DISABLE); // Winbutton14 set to enable touch, only need to do this once
  Display.img_Show(hndl, iWinbutton14);  // Winbutton14 show button, only do this once
  Display.img_ClearAttributes(hndl, iWinbutton15, I_TOUCH_DISABLE); // Winbutton15 set to enable touch, only need to do this once
  Display.img_Show(hndl, iWinbutton15);  // Winbutton15 show button, only do this once
...
void loop()
{
  ...
  if (n == iWinbutton13){
  Display.img_SetWord(hndl, iWinbutton13, IMAGE_INDEX, Wbs1+1);
  Display.img_Show(hndl, iWinbutton13);
  }
  if (n == iWinbutton14){
  Display.img_SetWord(hndl, iWinbutton14, IMAGE_INDEX, Wbs1+1);
  Display.img_Show(hndl, iWinbutton14);
  }
  if (n == iWinbutton15){
  Display.img_SetWord(hndl, iWinbutton15, IMAGE_INDEX, Wbs1+1);
  ...
  if ((GMT1 <= 16) && (GMT1 >= 1)){
    ...
    quad_offset = quad_offset & B00111100;
  }
  }
  if (n == iWinbutton2){
  if (Wbs1)
  ...
  if ((GMT2 <= 16) && (GMT2 >= 1)){
    quad_offset_2 = (byte) GMT2;
    quad_offset_2 = quad_offset_2 << 2;
    quad_offset = quad_offset & B00111101;
  }
  }
  if (n == iWinbutton4){
  if (Wbs1)
  ...
  if ((GMT3 <= 16) && (GMT3 >= 1)){
    quad_offset_3 = (byte) GMT3;
    quad_offset_3 = quad_offset_3 << 2;
    quad_offset = quad_offset & B00111110;
  }
  }
  if (n == iWinbutton6){
  if (Wbs1)
  ...
  if ((GMT4 <= 16) && (GMT4 >= 1)){
    quad_offset_4 = (byte) GMT4;
    quad_offset = quad_offset << 2;
    quad_offset = quad_offset & B00111111;
  }
  }
  if (n == iWinbutton8){
  if (Wbs1)
  ...
  if (n == iWinbutton9){
    if (Wbs1){
        Wbs1 = 0;
        quad_offset = B00000000;
        }
    else{
        Wbs1 = 2;
        quad_offset = B11111100;
        }
  ...
  if (n == iWinbutton10){
    if (Wbs1){
        Wbs1 = 0;
        quad_offset_2 = B00000000;
        }
    else{
        Wbs1 = 2;
        quad_offset_2 = B11111101;
        }
  ...
  if (n == iWinbutton11){
    if (Wbs1){
        Wbs1 = 0;
        quad_offset_3 = B00000000;
        }
    else{
        Wbs1 = 2;
        quad_offset_3 = B11111110;
        }
  ...
  if (n == iWinbutton12){
    if (Wbs1){
        Wbs1 = 0;
        quad_offset_4 = B00000000;
        }
    else{
        Wbs1 = 2;
        quad_offset_4 = B11111111;
        }
  ...
  if (n == iWinbutton13){
    ...
    Display.img_SetWord(hndl, iWinbutton13, IMAGE_INDEX, Wbs1);
    Display.img_Show(hndl, iWinbutton13);
  }
  if (n == iWinbutton14){
    if (Wbs1){
        ...
        SGMO4 = GMO4;
        SGMO5 = GMO5;
        squad_offset = quad_offset;
        //squad_offset_2 = quad_offset_2;
        //squad_offset_3 = quad_offset_3;
        //squad_offset_4 = quad_offset_4;
        }
    Display.img_SetWord(hndl, iWinbutton14, IMAGE_INDEX, Wbs1);
    Display.img_Show(hndl, iWinbutton14);
  }
  if (n == iWinbutton15){
    if (Wbs1){
        ...
        GMO4 = SGMO4;
        GMO5 = SGMO5;
        quad_offset = squad_offset;
        //quad_offset_2 = squad_offset_2;
        //quad_offset_3 = squad_offset_3;
        //quad_offset_4 = squad_offset_4;
        }
    else{
        Wbs1 = 2;
        GMT0 = SGMT0;
    ...
    Display.img_SetWord(hndl, iWinbutton15, IMAGE_INDEX, Wbs1);
    Display.img_Show(hndl, iWinbutton15);
  }
  }
  // Send out the byte to the Arduino, in any case
  Serial.println(quad_offset);
  Wire.beginTransmission(8); // we need an address value
  Wire.write(quad_offset);
  Wire.endTransmission();
  delay(500);
